...
const [selectedReviewImage, setSelectedReviewImage] = useState<string>(
  ...
  const fetchProducts = async () => {
    ...
    // В аналітику йде лише ВІДПРАВЛЕНИЙ пошук — той, що потрапив в адресу
    // (/search?q=…). Раніше подія висіла на самому наборі тексту, і кожна
    // пауза довша за дебаунс лишала окремий рядок: у журналі за 22.08 в
    // одну хвилину осіли «Деф», «Дефл», «Дефле», «Дефіле» замість одного
    // запиту. Звіт «чого не знайшли» від такого сміття втрачає сенс.
    //
    // Гортання сторінок теж не рахуємо: це не новий запит.
    if (currentPage === 1 && !error && data && searchQuery.trim() && searchQuery === urlQuery) {
