...
// Апостроф. У каталозі є «Екстер'єр» і «Інтер'єр», і до цієї правки пошук
// не знаходив їх НІ з апострофом, ні без: обидва написання з журналу
// покупців віддавали нуль. Тепер обидва зводяться до одного шаблону.
const uniq = (q: string) => [...new Set(values(q))];
assert.deepEqual(uniq('екстерєр'), ['екстер%єр'], 'апостроф не підставився');
assert.deepEqual(uniq("екстер'єр"), ['екстер%єр'], 'апостроф не став джокером');
assert.deepEqual(uniq('інтерєр'), ['інтер%єр']);
// джокер ставиться лише там, де апостроф можливий за правописом
assert.deepEqual(uniq('килим'), ['килим', 'коврик', 'кілім'], 'зайвий джокер у звичайному слові');
// а % від покупця так само вичищається — шукаємо те, що просили
assert.ok(!values('50%').some((v) => v.includes('%')), 'джокер покупця пройшов у запит');
