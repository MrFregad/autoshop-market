...
Поставщики товаров:
- **DD Audio** (ddaudio.com.ua) — основной каталог (~70 тыс. карточек), импорт через их официальный API, `supplier='ddaudio'`;
- **Dropt** (dropt.in.ua) — импорт из XML-фида + автопередача заказов, `supplier='dropt'`;
- **Koch Chemie** (автохимия) — собственные товары, `supplier IS NULL`, категории «Мийка авто», «Екстер'єр» и т.д. — импорты их не трогают.

---

## Интеграция с DD Audio (ddaudio.com.ua)

Основной каталог сайта. Товары тянутся из официального API поставщика
(розничные цены = РРЦ, скидки переносятся автоматически как `old_price`).

### Разовая настройка

1. **SQL-миграция** (один раз): Supabase → SQL Editor → вставить содержимое
   [`supabase/ddaudio_migration.sql`](supabase/ddaudio_migration.sql) → Run.
   Добавляет в `products` поля `marks`, `models`, `parent_id`, `short_title`
   и создаёт таблицу `car_models` (справочник для «Підбір за авто»).

2. **Секреты в GitHub** (репозиторий → Settings → Secrets and variables → Actions):
   - `DDAUDIO_API_TOKEN` — токен API из кабинета DD Audio;
   - `SUPABASE_SERVICE_KEY` — service-ключ Supabase (уже добавлен для Dropt).

### Импорт товаров

- **Автоматически**: GitHub Actions, каждый день в 07:30 по Киеву
  (workflow «DD Audio: імпорт товарів»). Обновляются цены, скидки и наличие;
  товары, пропавшие из прайса, помечаются «немає в наявності» (не удаляются).
  Если изменилось дерево категорий — workflow сам коммитит `src/catalogTree.ts`.
- **Вручную через GitHub**: Actions → «DD Audio: імпорт товарів» → Run workflow.
- **Вручную на компьютере** (переменные подхватываются из локального `.env`):
  ```bash
  node scripts/ddaudio-import.mjs --dry-run   # проверка без записи в базу
  node scripts/ddaudio-import.mjs             # боевой запуск
  ```

Особенности:
- лимит API поставщика — **1 запрос в 5 секунд**; скрипт сам держит паузу 5.5 с,
  весь импорт занимает 5-15 минут (~11 запросов + заливка в базу);
- один артикул в прайсе повторяется для каждой совместимой модели авто —
  скрипт группирует записи в одну карточку, а марки/модели складывает
  в массивы `marks`/`models` (на них работает фильтр «Підбір за авто»);
- цена сайта = розничная цена (РРЦ) DD Audio как есть; при действующей акции
  поставщика цена = акционная, старая показывается зачёркнутой. Если захочется
  своей наценки — переменная `DDAUDIO_MARKUP_PERCENT` (в процентах);
- категории API приходят по-русски — перевод на украинский правится в
  [`scripts/ddaudio-category-map.json`](scripts/ddaudio-category-map.json);
  скрипт предупреждает в логе, если появилась категория без перевода;
- `--replace-parsed` — **разовая** чистка старых товаров, залитых когда-то
  парсером CSV (удаляет строки `supplier IS NULL`, кроме категорий Koch Chemie).
  Сначала посмотреть, сколько удалится: `node scripts/ddaudio-import.mjs --dry-run --replace-parsed`.

Заказы: `/api/order` для товаров DD Audio показывает в Telegram
«🌐 Сайт: ddaudio.com.ua» и артикул поставщика (автопередачи заказа нет —
заказ у поставщика оформляется вручную по артикулу).
